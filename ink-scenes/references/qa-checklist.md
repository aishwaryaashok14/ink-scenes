...
- Readable words, labels, a title, or annotated arrows.
- Pure white background, paper texture, gradient, or vignette.
...
- Too plain: enrich the *texture* (more crosshatch detail on the subject), not the *inventory* — never fix plainness by adding props.
- Too busy: cut props to 1-3, push the scene smaller into more cream.
- Accent diluted: return everything but the pivot element to warm gray.
- Too cute: use the robot edit prompt — boxy panels, dot eyes, deadpan posture.
- Too diagrammatic: replace any arrows or floating panels with physical staging — things passed through windows, stacked on tables, fed into machines.
- Stray text: prefer a local edit to scribble or cream; regenerate if it is everywhere.

## Delivery test

A good image makes the reader smile slightly *before* they decode it, and decode it within two seconds *without* a caption.

If it needs the concept's name written on it to make sense, the metaphor failed — recompose, don't re-render.
